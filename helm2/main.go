...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		fooNs, err := corev1.NewNamespace(ctx, "fooNs", &corev1.NamespaceArgs{
		...
		foo, err := helmv3.NewRelease(ctx, "foo", &helmv3.ReleaseArgs{
			Chart:     pulumi.String("./charts/foo"),
			Namespace: fooNs.Metadata.Name(),
